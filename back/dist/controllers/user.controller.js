...
class UserController {
    constructor() {
        ...
        this.deleteCommentById = (req, res) => {
            ...
            server_1.pool.query(sql, [id], (err, comments) => {
                if (err) {
                    res.json({ error: 1, message: "Fatal error: " + err });
        ...
        this.rate = (req, res) => {
            ...
            server_1.pool.query(sql, [idUser, idRater], (err, data) => {
                ...
                if (data.length) {
                    ...
                    server_1.pool.query(sql, [rate, idUser, idRater], (err, data) => {
                        if (err) {
                            res.json({ error: 1, message: "Fatal error: " + err });
                            console.log('rate failed');
                            return;
                    ...
                    server_1.pool.query(sql, [idUser, idRater, rate], (err, data) => {
                        if (err) {
                            res.json({ error: 1, message: "Fatal error: " + err });
                            console.log('rate failed');
                            return;
        ...
        this.changePassword = (req, res) => {
            ...
            server_1.pool.query(sql, [newPassword, idUser, password], (err, comments) => {
                if (err) {
                    res.json({ error: 1, message: "Fatal error: " + err });
        ...
        this.updateGallery = (req, res) => {
            ...
            server_1.pool.query(sql, [idUser], (err, comments) => {
                ...
                images.forEach(element => {
                    ...
                    server_1.pool.query(sql1, [idUser, element], (err, comments) => {
                        if (err) {
                            res.json({ error: 1, message: "Fatal error: " + err });
                            console.log('updateGallery failed');
                            return;
        ...
        this.updateUser = (req, res) => {
            ...
            server_1.pool.query(sql, [username, email, firstname, lastname, birthday, location, phone, photo, backPhoto, idUser], (err, message) => {
                if (err) {
        ...
        this.forgotPasswordRequest = (req, res) => {
            ...
            server_1.pool.query(query, [req.body.email], (err, results) => {
                if (err) {
                    res.json({ error: 1, message: "Fatal error: " + err });
                    return;
                }
                ...
                server_1.pool.query(query, [req.body.email], (err) => {
                    if (err) {
                    ...
                    server_1.pool.query(query, [reset_token.token, req.body.email, expire_time], (err) => {
                        if (err) {
                            // console.error('Error inserting token into the database:', err);
                            // res.status(400).json({ 'message': 'error' });
                            res.json({ error: 1, message: "Fatal error: " + err });
        ...
        this.changeForgottenPassword = (req, res) => {
            ...
            server_1.pool.query(searchQuery, [email], (err, results) => {
                ...
                if (results.length > 0) {
                    ...
                    server_1.pool.query(updateQuery, [password, email], (err) => {
                        if (err) {
                            res.json({ error: 1, message: "Fatal error: " + err });
                            console.log('changeForgottenPassword failed');
                            return;
