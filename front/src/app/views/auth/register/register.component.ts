...
export class RegisterComponent implements OnInit {
  ...
  potvrdi(){
    ...
    if(this.korisnickoIme.length < 3 ){ this.poruka = "Ime je prekratko."; return; }
    if(this.korisnickoIme.length < 3 ){ this.poruka = "Prezime je prekratko."; return; }
    ...
    if (! "^\+381 \d{6}\d+$".match(this.brojTelefona)){ this.poruka = "Mobilni telefon nije u dobrom formatu."; return; }
    if ( ! "^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()\-_=+{};:,<.>]).{6,}$"  ) {this.poruka = "Lozinka mora imati barem \
