    ...
    update() {
        ...
        this.userService.getIdByEmail({ email: this.email }).subscribe((message: any) => {
            ...
            this.userService.getIdByUsername({ username: this.username }).subscribe((message: any) => {
                ...
                const normalizedLocation = this.location
                    .normalize("NFD")
                    .replace(/[\u0300-\u036f]/g, "")
                    .replace(/đ/g, "dj")
                    .replace(/Đ/g, "Dj");
                ...
                this.userService.updateUser(data).subscribe((message: any) => {
                    ...
                    this.jobService.updateUserProfessions({ idUser: this.idUser, professions: this.selectedProfessions, city: normalizedLocation }).subscribe((message: any) => {
                        if (message.error) {
